...
$pdf = new FPDF('P','mm','A4');
        ...
        $pdf->SetFont('Arial','',12);
        // $pdf->SetXY()
        $pdf->MultiCell(190,5,'         Yang bertanda tangan dibawah ini ' . $jabatan .' '. $dpByNik['dp_kecamatan'] . ' Kecamatan '.$dpByNik['dp_kecamatan'].', Kabupaten '.$dpByNik['dp_kabupaten'] .', Provinsi '.$dpByNik['dp_provinsi'] .' menerangkan yang sebenarnya bahwa :',0,1);
        ...
        $pdf->Ln();
        $pdf->MultiCell(190,5,'         Orang tersebut di atas adalah benar-benar warga '.$dpByNik['dp_kelurahan'].' yang saat ini Kartu Tanda Penduduk sedang dalam proses.',0,1);
        $pdf->Cell(190,7,'',0,1,'C');
