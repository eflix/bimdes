...
$pdf = new FPDF('P','mm','A4');
        ...
        $pdf->SetFont('Arial','',12);
        // $pdf->SetXY()
        $pdf->MultiCell(190,5,'         Yang bertanda tangan dibawah ini Kepala Desa '.$dpByNik['dp_kelurahan'].' Kecamatan '.$dpByNik['dp_kecamatan'].' Kabupaten '.$dpByNik['dp_kabupaten'] .' Provinsi '.$dpByNik['dp_provinsi'] .' menerangkan dengan sebenarnya bahwa :',0,1);
        ...
        $pdf->setX(30);$pdf->Cell(60,6,'3.   Tempat / Tanggal Lahir',0,0);
        $pdf->Cell(20,6,': '.$dpByNik['dp_tempat_lahir'],0,1);
        $pdf->setX(30);$pdf->Cell(60,6,'4.   Jenis Kelamin',0,0);
        $pdf->Cell(20,6,': '.$dpByNik['dp_gender'],0,1);
        $pdf->setX(30);$pdf->Cell(60,6,'5.   Agama',0,0);
        $pdf->Cell(20,6,': '.$dpByNik['dp_agama'],0,1);
        $pdf->setX(30);$pdf->Cell(60,6,'6.   Status',0,0);
        $pdf->Cell(20,6,': '.$dpByNik['dp_status_perkawinan'],0,1);
        $pdf->setX(30);$pdf->Cell(60,6,'7.   Pekerjaan',0,0);
        $pdf->Cell(20,6,': '.$dpByNik['dp_pekerjaan'],0,1);
        $pdf->setX(30);$pdf->Cell(60,6,'8. Kewarganegaraan',0,0);
        $pdf->Cell(20,6,': '.$dpByNik['dp_kewarganegaraan'],0,1);
        $pdf->setX(30);$pdf->Cell(60,6,'9.   Alamat',0,0);
        $pdf->Cell(20,6,': '.$dpByNik['dp_alamat'],0,1);

        $pdf->Ln();
        $pdf->MultiCell(190,5,'         Orang tersebut di atas adalah benar-benar warga kami yang bertempat tinggal di '.$dpByNik['dp_desa'].' Kecamatan '.$dpByNik['dp_kecamatan'].' dan tercatat dengan No. KK : '.$dpByNik['dp_no_kk'].' ',0,1);
        $pdf->Cell(190,7,'',0,1,'C');

        $pdf->MultiCell(190,5,'         Surat Keterangan ini dibuat untuk Keperluan : '. $keperluan,0,1);
        $pdf->Cell(190,7,'',0,1,'C');
